...
class FactoryConexion
{
    ...
    public static string CreateConnection()
    {
        ...
        {
            if (ConfigurationManager.ConnectionStrings[i].Name.Equals("Octopus.Properties.Settings.SqlServer", StringComparison.InvariantCultureIgnoreCase))
                existe = true;
        }

        if (!existe)
        {
            throw new Exception("No existe registrada en ConnectionStrings del app.config el Key Octopus.Properties.Settings.SqlServer!");
        }


        // Lee la conexion default
        conexion.AppendFormat("{0}", ConfigurationManager.ConnectionStrings["Octopus.Properties.Settings.SqlServer"].ConnectionString);
